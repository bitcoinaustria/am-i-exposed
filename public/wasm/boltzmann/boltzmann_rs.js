/* @ts-self-types="./boltzmann_rs.d.ts" */
 ...
 * @returns {any}
 */
export function compute_boltzmann(input_values, output_values, fee, max_cj_intrafees_ratio, timeout_ms) {
    ...
    const ret = wasm.compute_boltzmann(ptr0, len0, ptr1, len1, fee, max_cj_intrafees_ratio, timeout_ms);
    return ret;
}
 ...
 * @returns {any}
 */
export function dfs_finalize() {
    const ret = wasm.dfs_finalize();
    return ret;
}
 ...
 * @param {number} chunk_ms
 * @returns {any}
 */
export function dfs_step(chunk_ms) {
    const ret = wasm.dfs_step(chunk_ms);
    return ret;
}
 ...
 * @returns {any}
 */
export function prepare_boltzmann(input_values, output_values, fee, max_cj_intrafees_ratio, timeout_ms) {
    const ptr0 = passArray64ToWasm0(input_values, wasm.__wbindgen_malloc);
    const len0 = WASM_VECTOR_LEN;
    const ptr1 = passArray64ToWasm0(output_values, wasm.__wbindgen_malloc);
    const len1 = WASM_VECTOR_LEN;
    const ret = wasm.prepare_boltzmann(ptr0, len0, ptr1, len1, fee, max_cj_intrafees_ratio, timeout_ms);
    return ret;
}
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('boltzmann_rs_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
