...
export default function FaqPage() {
  ...
  const filteredItems = search.trim()
    ? FAQ_ITEMS.filter((item) => {
        ...
        const question = t(item.qKey, { defaultValue: DEFAULTS[item.qKey] }).toLowerCase();
        const answer = t(item.aKey, { defaultValue: DEFAULTS[item.aKey] }).toLowerCase();
    ...
    <PageShell backLabel={t("faq.back", { defaultValue: "Back to scanner" })}>
        ...
        <div className="relative">
          <input
            ...
            className="w-full px-4 py-3 rounded-lg bg-surface-inset border border-card-border text-sm text-foreground placeholder:text-muted/60 focus:outline-none focus:border-bitcoin/50 transition-colors"
            aria-label={t("faq.searchLabel", { defaultValue: "Search FAQ" })}
          />
          {search && (
            <span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-muted">
              {filteredItems.length} {t("faq.results", { defaultValue: "results" })}
            </span>
          )}
        </div>
            ...
            >
              ...
              <AnimatePresence>
                {open === item.id && (
                  ...
                  >
                    <div id={`faq-panel-${item.id}`} role="region" aria-labelledby={`faq-trigger-${item.id}`} className="px-5 pb-4 text-sm text-muted leading-relaxed space-y-3">
